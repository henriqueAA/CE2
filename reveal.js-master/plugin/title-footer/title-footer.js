...
title_footer.initialize=function(title,background)
{
	...
	link.href="../../reveal.js-master/plugin/title-footer/title-footer.css";
	link.type="text/css";
	link.rel="stylesheet";
	...
	else
	{
		...
		if (title_elements.length>0)
		{
			...
			{
				this.title=this.title+'<span class="contact">'+title_elements[title_elements_index].textContent+'</span>';
			};
		};
	};
	...
	span_element.appendChild(document.createTextNode(this.title));
	title_footer_p.innerHTML = span_element.textContent;
